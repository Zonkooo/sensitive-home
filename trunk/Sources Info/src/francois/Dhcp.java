				...
				while (stLog.hasMoreTokens()) {
					...
					if(stCouranteLog.length()==17){
						...
						while (stDhcp.hasMoreTokens()) {
							...
							if(stCouranteDhcp.length()==17){

								//est-ce que la mac existe déjà?
								if(stCouranteDhcp.equals("stCouranteLog")){
									System.out.println(stCouranteDhcp+" est déjà enregistrée");
								} else {
									//on écrit dans le dhcpd.conf
									try {
										FileWriter fw = new FileWriter ("/etc/dhcp3/dhcpd.conf");
